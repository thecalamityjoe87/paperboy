...
namespace Paperboy {
    ...
    public class RssSourceStore : GLib.Object {
        ...
        private void init_database() {
            ...
            string add_custom_name = "ALTER TABLE rss_sources ADD COLUMN custom_name TEXT;";
            rc = db.exec(add_custom_name, null, out errmsg);
            if (rc != Sqlite.OK && errmsg != null && !errmsg.down().contains("duplicate column")) {
        ...
        public Gee.ArrayList<RssSource> get_all_sources() {
            ...
            sources.sort((a, b) => { return SortUtils.compare_titles(a.custom_name ?? a.name, b.custom_name ?? b.name); });
        ...
        // null/empty clears the override and falls back to the default name.
        public bool set_custom_name(string url, string? custom_name) {
            if (db == null) {
                GLib.warning("Database not initialized");
                return false;
            }

            string sql = "UPDATE rss_sources SET custom_name = ? WHERE url = ?;";
            Sqlite.Statement stmt;
            int rc = db.prepare_v2(sql, -1, out stmt);
            if (rc != Sqlite.OK) {
                GLib.warning("Failed to prepare statement: %s", db.errmsg());
            ...
            string? trimmed = custom_name != null ? custom_name.strip() : null;
            if (trimmed != null && trimmed.length > 0) {
                stmt.bind_text(1, trimmed);
            } else {
                stmt.bind_null(1);
            }
            stmt.bind_text(2, url);
            ...
            if (stmt.step() != Sqlite.DONE) {
                GLib.warning("Failed to update custom name: %s", db.errmsg());
                return false;
            }
