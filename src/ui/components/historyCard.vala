...
public class HistoryCard : GLib.Object {
    ...
    // Measured directly off the reference mockup (277px wide x 91px tall
    // per card = 3.04:1). Height is derived from col_w using this ratio,
    // not a flat pixel guess, so the row keeps the same proportions the
    // mockup showed no matter how wide the column ends up being.
    public const double CARD_ASPECT_RATIO = 3.04;
