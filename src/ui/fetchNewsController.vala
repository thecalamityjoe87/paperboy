...
public class FetchNewsController {
    ...
    public static void fetch_news(NewsWindow win) {
        ...
        try { if (win.loading_state != null) win.loading_state.begin_fetch(); } catch (GLib.Error e) { }
        try { win.update_content_header_now(); } catch (GLib.Error e) { }
        
        // Capture a strong reference to `win` so the wrapped callbacks hold
        // the NewsWindow alive while they're queued.
        var self_ref = win;
        ...
        // to the window for safe async access.
        // IMPORTANT: take an extra strong reference to `ctx` so callbacks
        // which capture it don't access a freed object if a newer fetch
        // replaces the static `_current_context` (which would otherwise
        // drop the last strong reference and free the object). We'll
        // unref this extra reference after a safety timeout.
        var ctx = FetchContext.begin_new(win);
        try { ctx.ref(); } catch (GLib.Error e) { }
        ...
        if (win.loading_state != null) {
            win.loading_state.initial_reveal_timeout_id = Timeout.add(NewsWindow.INITIAL_MAX_WAIT_MS, () => {
                ...
                if (!w.loading_state.initial_items_populated) {
                    try {
                        if (w.loading_state.network_failure_detected) {
        ...
        Timeout.add(NewsWindow.INITIAL_MAX_WAIT_MS + 2000, () => {
            try { self_ref.unref(); } catch (GLib.Error e) { }
            return false;
        });

        // Unref the extra ctx reference after a bounded time so we don't leak
        // if something goes wrong. This keeps the ctx alive for the duration
        // of the initial fetch window and any queued Idle callbacks.
        Timeout.add(NewsWindow.INITIAL_MAX_WAIT_MS + 2000, () => {
            try { ctx.unref(); } catch (GLib.Error e) { }
            return false;
        });
        ...
        ClearItemsFunc wrapped_clear = () => {
            ...
            Idle.add(() => {
                ...
                w.article_manager.clear_article_buffer();
                
                // 4. Remove "No more articles" message
                w.layout_manager.remove_end_feed_message();
                
                // 5. Ensure any load-more button managed by ArticleManager is removed
                w.article_manager.clear_load_more_button();
                
                // 6. Clear image bookkeeping
                try { if (w.view_state != null) w.view_state.url_to_picture.clear(); } catch (GLib.Error e) { }
                try { w.image_manager.hero_requests.clear(); } catch (GLib.Error e) { }
                
                // 7. Reset remaining articles state
                if (w.article_manager.remaining_articles != null) {
                    w.article_manager.remaining_articles.clear();
                }
                w.article_manager.remaining_articles_index = 0;
        ...
        if (win.category_manager.is_frontpage_view()) {
            ...
            try { self_ref.header_manager.setup_multi_source_header(); } catch (GLib.Error e) { }
            used_multi = true;
            ...
            NewsService.fetch(win.prefs.news_source, "frontpage", current_search_query, win.session, wrapped_set_label, wrapped_clear, wrapped_add);
            
            // Schedule badge refresh for frontpage
            if (self_ref.sidebar_manager != null) {
                self_ref.sidebar_manager.schedule_badge_refresh("frontpage", my_seq);
            }
            return;
        }
        ...
        if (win.category_manager.is_topten_view()) {
            ...
            NewsService.fetch(win.prefs.news_source, "topten", current_search_query, win.session, wrapped_set_label, wrapped_clear, wrapped_add);

            // Schedule badge refresh for topten
            if (self_ref.sidebar_manager != null) {
                self_ref.sidebar_manager.schedule_badge_refresh("topten", my_seq);
            }
            return;
        }
        ...
        if (total_sources > 1 || (is_myfeed_mode && custom_rss_sources != null && custom_rss_sources.size > 0)) {
            ...
            if (srcs.size == 0) {
                ...
                if (!skip_builtin) {
                    foreach (var s in use_srcs) {
                        if (is_myfeed_mode) {
                            foreach (var cat in myfeed_cats) {
                                NewsService.fetch(s, cat, current_search_query, win.session, label_fn, no_op_clear, wrapped_add);
                            }
                        } else {
                            NewsService.fetch(s, win.prefs.category, current_search_query, win.session, label_fn, no_op_clear, wrapped_add);
                        }
                    }
                }

                // Schedule badge refresh for non-myfeed categories after articles register
                if (!is_myfeed_mode && self_ref.sidebar_manager != null) {
                    self_ref.sidebar_manager.schedule_badge_refresh(win.prefs.category, my_seq);
                }
            ...
            if (is_myfeed_mode) {
                ...
                if (!win.prefs.myfeed_custom_only) {
                    foreach (var cat in myfeed_cats) {
                        NewsService.fetch(win.effective_news_source(), cat, current_search_query, win.session, label_fn, no_op_clear, wrapped_add);
                    }
                }
                ...
                if (self_ref.sidebar_manager != null) {
                    self_ref.sidebar_manager.schedule_badge_refresh("myfeed", my_seq);
                }
            } else {
                try { wrapped_clear(); } catch (GLib.Error e) { }
                NewsService.fetch(
    ...
    public static bool handle_local_news(
        ...
        for (int i = 0; i < lines.length; i++) {
            ...
            found_feed = true;
            try { win.article_manager.featured_used = true; } catch (GLib.Error e) { }
