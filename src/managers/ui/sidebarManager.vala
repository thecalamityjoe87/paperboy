...
public class SidebarManager : GLib.Object {
    ...
    private void save_local_news_state() {
        var prefs = NewsPreferences.get_instance();
        prefs.sidebar_local_news_expanded = local_news_expanded;
        prefs.save_config();
    }
     ...
     */
    public void set_section_expanded(string section_id, bool expanded) {
        if (section_id == "followed_sources") {
            if (followed_sources_expanded == expanded) return;
            followed_sources_expanded = expanded;
            save_followed_sources_state();
        } else if (section_id == "local_news_entry") {
            if (local_news_expanded == expanded) return;
            local_news_expanded = expanded;
            save_local_news_state();
        } else if (section_id == "popular_categories") {
            if (popular_categories_expanded == expanded) return;
            popular_categories_expanded = expanded;
        ...
        expanded_state_changed(section_id, expanded);
    }
    ...
    public Gee.ArrayList<SidebarSectionData?> get_sidebar_sections() {
        ...
        // Section 1: Special items (Front Page, My Feed, Saved, ...)
        ...
        // Local News: one row per saved city. The active city keeps the
        ...
        var prefs = NewsPreferences.get_instance();
        var active_area = prefs.get_active_local_area();
        foreach (var area in prefs.get_local_areas()) {
            bool is_active = active_area != null && area.key == active_area.key;
            var item = create_item_data(area.name,
                is_active ? "local_news" : area.id, SidebarItemType.SPECIAL);
            item.icon_key = "local_news";
            local_section.items.add(item);
        }
    ...
    private SidebarItemData create_rss_item_data(Paperboy.RssSource source) {
        ...
        item.title = source.get_display_name();
    ...
    private int get_unread_count_for_item(string id, SidebarItemType type) {
        ...
        // Local News cities: the active row is "local_news", the rest use their own id.
        if (id == "local_news") {
            var local_area = window.prefs.get_active_local_area();
            return local_area != null ? get_unread_count_for_local_area(local_area.id) : 0;
        }
        if (id.has_prefix(LocalArea.ID_PREFIX)) {
            return get_unread_count_for_local_area(id);
        }
    ...
    // Same rules as a feed: "--" until first opened, then that city's own count.
    private int get_unread_count_for_local_area(string area_id) {
        if (!is_category_visited(area_id)) return -1;
        return window.article_state_store.get_unread_count_for_category(area_id);
    }
    ...
    public void handle_item_activation(string id, string title) {
        ...
        // Switching cities: make it active, then open Local News for it.
        if (id.has_prefix(LocalArea.ID_PREFIX)) {
            window.prefs.active_local_area_key = id.substring(LocalArea.ID_PREFIX.length);
            id = "local_news";
            Idle.add(() => {
                rebuild_sidebar();
                return false;
    ...
    private RssSourceItemData create_rss_source_item_data(Paperboy.RssSource source) {
        ...
        item.url = source.url;
        item.display_name = source.get_display_name();
    ...
    public void update_badge_for_category(string category_id) {
        ...
        if (window.article_state_store != null) {
            ...
            } else if (category_id == "local_news") {
                unread_count = get_unread_count_for_item(category_id, SidebarItemType.SPECIAL);
