...
namespace Paperboy {
    public class RssSource : GLib.Object {
        ...
        public string get_display_name() {
            if (custom_name != null && custom_name.strip().length > 0) return custom_name;
            string? meta = SourceMetadata.get_display_name_for_source(name);
            if (meta != null && meta.length > 0) return meta;
            return name;
        }
    }
}
